...
function SubscriptionDetails({ setShowCart }) {
  ...
  useEffect(() => {
    fetchData(`/subscriptions`)
      .then(data => setRelatedSubscriptions(
        data.filter(s => s.id !== parseInt(id)).slice(0, 4)
      ))
      .catch(err => {
        console.error(err);
        setError("Une erreur est survenue lors du chargement des abonnements suggérés.");
      });
  }, [id]);
  ...
  return (
    <div className="bg-[#FFF7F0] min-h-screen">
      ...
      {/* Route */}
      <div>
        <p><span className="cursor-pointer hover:underline" onClick={() => navigate('/subscriptions')}>Nos abonnements</span> / {subscription.name}</p>
      </div>
      {/* Détails principaux */}
        ...
        <div className="w-[34%]">
          <div className="flex flex-row justify-between">
            <div className="flex flex-col">
              <h2 className="!text-[36px] leading-[36px] text-[#1B1B1B]">{subscription.name}</h2>
              <p className="!text-[18px] text-[#666] mt-1">{subscription.type || "Type d'abonnement"}</p>
